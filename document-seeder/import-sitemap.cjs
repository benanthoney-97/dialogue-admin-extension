...
const PROVIDER_ID = Number(process.env.SITEMAP_PROVIDER_ID || 12);
...
const insertFeed = async (feedUrl, lastModified) => {
  const { data, error } = await supabase
    ...
    .upsert(
      ...
      { onConflict: "provider_id,feed_url" }
    )
    .select("id")
    .maybeSingle();
...
const main = async () => {
  ...
  if (!indexUrl) {
    console.error("Usage: node document-seeder/import-sitemap.js <sitemap_index_url>");
    process.exit(1);
  }
  try {
